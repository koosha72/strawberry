...
namespace Strawberry.OpenGL.Sound
{
    public class SoundBuffer : Base, ISoundBuffer
    {
        ...
        public float Seconds
        {
            get
            {
                var size = AL.GetBuffer(ID, ALGetBufferi.Size);
                if (size == 0)
                    return 0;
                var frequency = (float)AL.GetBuffer(ID, ALGetBufferi.Frequency);
                var channels = (float)AL.GetBuffer(ID, ALGetBufferi.Channels);
                var bits = (float)AL.GetBuffer(ID, ALGetBufferi.Bits);
