...
namespace Strawberry.OpenGL.Sound
{
    public class SoundManager : Base, ISoundManager
    {
        ...
        internal Voice Play(SoundBuffer buffer, float frequencyRatio = 1.0f, bool loop = false)
        {
            ...
            AL.Source(source, ALSourcef.Pitch, frequencyRatio);
            if (loop)
        ...
        internal Voice3D Play(SoundBuffer buffer, Voice3DSettings settings, float frequencyRatio = 1.0f, bool loop = false)
        {
            ...
            AL.Source(source, ALSourcei.Buffer, buffer.ID);
            AL.Source(source, ALSourcef.Pitch, frequencyRatio);
            AL.Source(source, ALSource3f.Position, settings.Position.X, settings.Position.Y, settings.Position.Z);
            AL.Source(source, ALSource3f.Velocity, settings.Velocity.X, settings.Velocity.Y, settings.Velocity.Z);
            AL.Source(source, ALSource3f.Direction, settings.Direction.X, settings.Direction.Y, settings.Direction.Z);
            if (loop)
                AL.Source(source, ALSourceb.Looping, true);
            AL.SourcePlay(source);
            return v;
